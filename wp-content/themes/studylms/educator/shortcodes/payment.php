...
if ( $payment_id ) {
	...
	?>
	<h2><?php esc_html_e( 'Payment Summary', 'studylms' ); ?></h2>

	<dl id="payment-details" class="edr-dl">
		<dt class="payment-id"><?php esc_html_e( 'Payment', 'studylms' ); ?></dt>
		<dd><?php echo intval( $payment->ID ); ?></dd>

		<dt class="payment-date"><?php esc_html_e( 'Date', 'studylms' ); ?></dt>
		<dd><?php echo esc_html( date_i18n( get_option( 'date_format' ), strtotime( $payment->payment_date ) ) ); ?></dd>

		<dt class="payment-status"><?php esc_html_e( 'Payment Status', 'studylms' ); ?></dt>
		<dd>
			<?php
	...
	<table class="edr-payment-table">
		<thead>
			<tr>
				<th><?php esc_html_e( 'Curso', 'studylms' ); ?></th>
				<th><?php esc_html_e( 'Valor', 'studylms' ); ?></th>
			</tr>
		</thead>
		<tbody>
			<tr>
	...
	print_r($payment);
			if ( $payment->tax > 0.0 ) {
				echo '<dt class="payment-subtotal">' . esc_html__( 'Subtotal', 'studylms' ) .'</dt><dd>' . edr_format_price( $payment->amount - $payment->tax, false ) . '</dd>';
		...
		<dt class="payment-total"><?php esc_html_e( 'Total', 'studylms' ); ?></dt>
	...
	if ( $payments_page ) {
		echo '<p class="forward">' . sprintf(__( 'Go to %s', 'studylms' ), '<a class="btn btn-theme" href="' . esc_url( get_permalink( $payments_page->ID ) ) .
	...
	if ( EDR_PT_COURSE == $post->post_type ) {
		...
		if ( 'closed' == $course_status ) {
			echo '<p>' . esc_html__( 'Registration for this course is closed.', 'studylms' ) . '</p>';
			return;
		}
		...
		if ( $user_id ) {
			...
			if ( ! empty( $payments ) ) {
				echo '<p>' . esc_html__( 'The payment for this course is pending.', 'studylms' ) . '</p>';
				...
				if ( $payments_page ) {
					...
					echo '<p>' . sprintf(__( 'Go to %s', 'studylms' ), $payments_link ) . '</p>';
				}
	...
	if ( ! $user_id ) {
		...
		echo '<p>' . esc_html__( 'Already have an account?', 'studylms' ) . ' <a href="' .
			esc_url( $login_url ) . '">' . esc_html__( 'Log in', 'studylms' ) . '</a></p>';
	}
	...
	?>
		<form id="edr-payment-form" class="edr-form" action="<?php echo esc_url( $form_action ); ?>" method="post">
			...
			<div class="edr-form__fields">
				<div class="edr-form__legend"><?php esc_html_e( 'Realizar el curso', 'studylms' ); ?></div>
				...
				<?php if ( $args['price'] && ! empty( $gateways ) ) : ?>
					<div class="edr-field<?php if ( in_array( 'empty_payment_method', $error_codes ) ) echo ' edr-field_error'; ?>">
						<div class="edr-field__label">
							<label><?php esc_html_e( 'Payment Method', 'studylms' ); ?><span class="required">*</span></label>
						</div>
									...
									foreach ( $gateways as $gateway_id => $gateway ) {
										...
										<li>
											<label class="edr-radio">
												<input type="radio" name="payment_method" value="<?php echo esc_attr( $gateway_id ); ?>"<?php echo trim($checked); ?>><span><?php echo esc_html( $gateway->get_title() ); ?></span>
											</label>
										</li>
										<?php
									}
			...
			<div class="edr-form__actions">
				<button type="submit" class="edr-button"><?php esc_html_e( 'Comenzar', 'studylms' ) ?></button>
			</div>
		</form>
	<?php
}
