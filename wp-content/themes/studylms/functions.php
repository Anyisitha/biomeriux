...
define( 'STUDYLMS_THEME_VERSION', '1.15' );
...
function save_favorite_color_account_details( $user_id ) {
    ...
    if( isset( $_POST['favorite_color2'] ) )
        ...
        update_user_meta( $user_id, 'description', sanitize_text_field( $_POST['account_description'] ) );

	if( isset( $_POST['account_phone'] ) )
        update_user_meta( $user_id, 'apus_edr_info_mobile', sanitize_text_field( $_POST['account_phone'] ) );

	if( isset( $_POST['account_skype'] ) )
        update_user_meta( $user_id, 'apus_edr_info_skype', sanitize_text_field( $_POST['account_skype'] ) );

	if( isset( $_POST['account_url'] ) )
        update_user_meta( $user_id, 'url', sanitize_text_field( $_POST['account_url'] ) );

	if( isset( $_POST['account_facebook'] ) )
        update_user_meta( $user_id, 'apus_edr_info_facebook', sanitize_text_field( $_POST['account_facebook'] ) );

	if( isset( $_POST['account_twitter'] ) )
        update_user_meta( $user_id, 'apus_edr_info_twitter', sanitize_text_field( $_POST['account_twitter'] ) );

	if( isset( $_POST['account_linkedin'] ) )
        update_user_meta( $user_id, 'apus_edr_info_linkedin', sanitize_text_field( $_POST['account_linkedin'] ) );

	if( isset( $_POST['account_youtube'] ) )
        update_user_meta( $user_id, 'apus_edr_info_youtube', sanitize_text_field( $_POST['account_youtube'] ) );


	if( isset( $_POST['account_cargo'] ) )
        update_user_meta( $user_id, 'apus_edr_info_cargo', sanitize_text_field( $_POST['account_cargo'] ) );

	if( isset( $_POST['account_institucion'] ) )
        update_user_meta( $user_id, 'apus_edr_info_institucion', sanitize_text_field( $_POST['account_institucion'] ) );

	if( isset( $_POST['account_ciudad'] ) )
        update_user_meta( $user_id, 'apus_edr_info_ciudad', sanitize_text_field( $_POST['account_ciudad'] ) );
...
function configurar_usuario(){
  ...
  $existe_cargo = $wpdb->get_results("SELECT `meta_value` FROM `wprueba_usermeta` WHERE `meta_key` = 'first_name' AND `user_id` = " . $user_id);
  if($existe_cargo[0]->meta_value == ''){
    $desactivar = $wpdb->get_results("INSERT INTO `wprueba_usermeta`(`user_id`, `meta_key`, `meta_value`) VALUES (". $user_id .",'erf_active','0')");

    $nombres_acf = get_field("nombres", "user_{$user_id}");
    $nombre = $wpdb->get_results("UPDATE `wprueba_usermeta` SET `meta_value`='". $nombres_acf ."' WHERE  `user_id` = " . $user_id . " AND `meta_key` = 'first_name'");

    $apellidos_acf = get_field("apellidos", "user_{$user_id}");
    $apellidos = $wpdb->get_results("UPDATE `wprueba_usermeta` SET `meta_value`='". $apellidos_acf ."' WHERE  `user_id` = " . $user_id . " AND `meta_key` = 'last_name'");

    $cargo_acf = get_field("cargo", "user_{$user_id}");
    $cargo = $wpdb->get_results("INSERT INTO `wprueba_usermeta`(`user_id`, `meta_key`, `meta_value`) VALUES (". $user_id .",'apus_edr_info_cargo','". $cargo_acf ."')");

    $institucion_acf = get_field("institucion", "user_{$user_id}");
    $institucion = $wpdb->get_results("INSERT INTO `wprueba_usermeta`(`user_id`, `meta_key`, `meta_value`) VALUES (". $user_id .",'apus_edr_info_institucion','". $institucion_acf ."')");

    $celular_acf = get_field("celular", "user_{$user_id}");
    $celular = $wpdb->get_results("INSERT INTO `wprueba_usermeta`(`user_id`, `meta_key`, `meta_value`) VALUES (". $user_id .",'apus_edr_info_mobile','". $celular_acf ."')");

    $ciudad_acf = get_field("ciudad", "user_{$user_id}");
    $ciudad = $wpdb->get_results("INSERT INTO `wprueba_usermeta`(`user_id`, `meta_key`, `meta_value`) VALUES (". $user_id .",'apus_edr_info_ciudad','". $ciudad_acf ."')");
...
//add_action('user_register','configurar_usuario');
add_action('wp_enqueue_scripts', 'dcms_insertar_js');

function dcms_insertar_js(){
    wp_register_script('intentoscr',get_stylesheet_directory_uri(). '/js/cr-intentos.js', array('jquery'), '9', true );
    wp_enqueue_script('intentoscr');
    wp_localize_script('intentoscr','intentos_vars',['ajaxurl'=>admin_url('admin-ajax.php')]);
}
//add_action('wp_ajax_nopriv_enviarEventos','dcms_enviar_contenido');
add_action('wp_ajax_enviarIntentos','enviar_intentos');

function enviar_intentos() {
  echo 'Estoy listo para escuchar';
}
