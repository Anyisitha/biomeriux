...
function studylms_educator_get_courses( $course_type, $number = -1, $author = false, $ids = array() ) {
	switch ( $course_type ) {
	    case 'most_recent' : 
	       $args = array( 
...
function studylms_educator_get_meta($key) {
	$prefix = 'apus_educator_';
	return get_post_meta( get_the_ID(), $prefix.$key, true );
}

function studylms_educator_get_students_by_course($course_id) {
	$obj = Edr_Entries::get_instance();
	$entries = $obj->get_entries( array('course_id' => $course_id, 'entry_status' => 'inprogress') );
	return $entries;
}

if ( !function_exists('studylms_course_content_class') ) {
	function studylms_course_content_class( $class ) {
		$page = 'archive';
		if ( is_singular( EDR_PT_COURSE ) ) {
            $page = 'single';
        }
		if ( studylms_get_config('course_'.$page.'_fullwidth') ) {
			return 'container-fluid';
		}
		return $class;
	}
}
add_filter( 'studylms_course_content_class', 'studylms_course_content_class', 1 , 1  );

if ( !function_exists('studylms_get_course_layout_configs') ) {
	function studylms_get_course_layout_configs() {
		$page = 'archive';
		if ( is_singular( EDR_PT_COURSE ) || is_singular( EDR_PT_LESSON ) || is_singular( EDR_PT_MEMBERSHIP ) ) {
            $page = 'single';
        }
		$left = studylms_get_config('course_'.$page.'_left_sidebar');
		$right = studylms_get_config('course_'.$page.'_right_sidebar');

		switch ( studylms_get_config('course_'.$page.'_layout') ) {
		 	case 'left-main':
...
function studylms_edr_format_price($formatted, $currency, $price) {
	return '<span class="price-wrapper">'.$formatted.'</span>';
}
add_filter( 'edr_format_price', 'studylms_edr_format_price', 10, 3 );

function studylms_set_course_views($content) {
	global $post;
...
add_filter( 'the_content', 'studylms_set_course_views' );

function studylms_get_course_views() {
	global $post;
	return get_post_meta( $post->ID, 'edr_views_count', true );
}

function studylms_educator_search_filter($query) {
...
add_filter('pre_get_posts', 'studylms_educator_search_filter');

function studylms_educator_display_lessons( $course_id ) {
...
add_filter( 'template_include', 'studylms_educator_templates' );
function studylms_educator_templates( $template ) {
