...
<?php if ( ! empty( $questions ) ) : ?>
    ...
    //echo 'numero max de oportunidades: ' . $max_attempts_number;

		// Get the number of times a user attempted to complete this quiz..
		$pre_attempts_number = $wpdb->get_results("SELECT `intentos` FROM `wprueba_edr_grades` WHERE `user_id` = '{$user_id}' AND `entry_id`= '{$entry_id}'  AND `lesson_id` = {$post_id} ");
    //echo 'abajo';
    //echo var_dump($pre_attempts_number); 
    if(sizeOf($pre_attempts_number) == 0 ){
      $attempts_number = 0;
    }else{
      $attempts_number = intval($pre_attempts_number[0]->intentos);
    }
    //echo var_dump("SELECT `intentos` FROM `wprueba_edr_grades` WHERE `user_id` = '{$user_id}' AND `entry_id`= '{$entry_id}'  AND `lesson_id` = {$post_id} ");
    //echo var_dump($attempts_number);
    //echo 'numero de oportunidades: ' . $attempts_number;

		// Check if a user has enough attempts to complete this quiz.
		$can_attempt = $attempts_number < $max_attempts_number;
    //echo var_dump($can_attempt);
	...
	<div id="edr-quiz-<?php echo intval( $post_id ); ?>" class="edr-quiz <?php echo $do_quiz ? 'edr-quiz_editable' : 'edr-quiz_done'; ?>">
		...
		<div class="edr-quiz__attempts">
			...
			<?php if ( $can_attempt && ! $do_quiz ) : ?>
				<a class="edr-quiz__attempts__try-again" href="<?php echo esc_url( add_query_arg( 'try_again', 'true', get_permalink() ) ); ?>#edr-quiz"><?php _e( 'Inténtalo de nuevo', 'educator' ); ?></a>
			<?php endif; ?>
		</div>
